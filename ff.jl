...
function quat_to_mat2(q)
    q0=q[1]
    q=[q[2];q[3];q[4]]
    return (q0*q0-q'*q)*I+2*q*q'+2*q0*[q]x
end
...
function quat_mult(a,b)
    ...
    p0=b[1]
    p=[b[2];b[3];b[4]]
    r = [q0*p0-q'*p; q0*p+p0*q+cross(q,p)]
    return r
end

function quat_mult2(a,b)
    q0=a[1]
    q1=a[2]
    q2=a[3]
    q3=a[4]
    return [q0 -q1 -q2 -q3; q1 q0 -q3 q2; q2 q3 q0 -q1; q3 -q2 q1 q0]*b
end
...
println("\n quat_to_mat =\n", quat_to_mat(test_q))
println("\n quat_to_mat2 =\n", quat_to_mat2(test_q))
println("\n quat_mult =\n", quat_mult(test_q, test_q))
println("\n quat_mult2 =\n", quat_mult2(test_q, test_q))
